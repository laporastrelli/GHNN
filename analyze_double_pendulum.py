...
def generate_nn_paths_list(names):
    ...
    for name in names:
        nn_paths = []
        for i in range(1,51):
            nn_paths.append(os.path.join('..', 'NeuralNets_GHNN', 'double_pendulum', name, f'nn_{i}'))
...
if __name__ == '__main__':
    ...
    ghnn.plotting.predict_pendulum(data_path, store_name, 458, nn_paths, save_name=os.path.join(figures_path, 'doub_pend_plots.png'), **kwargs)
