...
os.mkdir(os.path.join('..', 'NeuralNets_GHNN', 'half_rand_pendulum'))
nn_types = ['GHNN', 'SympNet', 'MLP', 'HenonNet', 'double_HenonNet']

for nn_type in nn_types:
    nn_path = os.path.join('..', 'NeuralNets_GHNN', 'half_rand_pendulum', nn_type)
    os.mkdir(nn_path)
    ...
    data_path = os.path.join(data_path, 'Data_half_rand', 'pend_training.h5.1')
